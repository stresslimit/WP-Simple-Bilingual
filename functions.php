...
define( 'sld_bi_default_lang', 'fr' );

add_action( 'init', 'sld_bi_setup');
function sld_bi_setup() {
	if ( is_admin() || is_login_page() ) {
		load_theme_textdomain( 'sld_bi', TEMPLATEPATH.'/lang' );
	} else {
	...
	// don't allow people to go to home page without language
	// if ( empty($lang) ) { wp_redirect( '/'. sld_bi_default_lang, 301 ); exit; }
	
	// make sure the first url segment is /en/ or /fr/, if not just add /fr/ in front of the request uri
	// but ignore if it's a feed, ie. the last uri segment is /feed [for eblasts reading from mailchimp]
	if ( !empty($lang)  &&  ( $lang!='fr' && $lang!='en' )  &&  $last!='feed' ) { wp_redirect( '/' . $default_lang . $orig_uri, 301 ); exit; }
	...
	load_theme_textdomain( 'asgaard', TEMPLATEPATH.'/lang' );
...
function sld_lang_switcher_link( $args = null ) {
	...
	$lang_link = str_replace( '/'.$lang.'/', '/'.$lang_switcher_link[$lang].'/', $args['permalink'] );
...
function sld_rewrite_rules($aRules) {
	...
	$aNewRules = array(
		...
		'(en|fr)/(clients)/([^/]+)/?$' => 'index.php?post_type=client&client=$matches[3]&name=$matches[3]&lang=$matches[1]',

		'(en|fr)/(property)/([^/]+)/?$' => 'index.php?post_type=property&property=$matches[3]&name=$matches[3]&lang=$matches[1]',

		// '(en|fr)/daily_eblasts/feed/?$' => 'index.php?post_type=daily_eblasts&feed=rss',
		'(en|fr)/(daily_eblasts)/([^/]+)/?$' => 'index.php?post_type=daily_eblast&daily_eblast=$matches[3]&name=$matches[3]&lang=$matches[1]',

		'(en|fr)/(properties)/([^/]+)/?$' => 'index.php?pagename=$matches[2]&lang=$matches[1]&property_type=$matches[3]',
